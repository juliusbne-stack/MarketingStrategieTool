...
import {
  ...
  PHASE3_STEP_CONFIG,
  type Question,
} from "./phase3-questions";
import { ConfigDrivenWizard } from "@/app/wizard/_shared/ConfigDrivenWizard";
import { generatePhase3PositioningBrandTwoVariants } from "@/app/actions/phase3-actions";

interface InputWizardProps {
  projectId: number;
}

/** Validates a single answer for Phase 3 question types. */
            ...
            <label key={o.id} className="flex items-center gap-2 cursor-pointer">
              <Checkbox
                ...
                onCheckedChange={(checked) => {
                  if (checked) {
                    if (arr.length < max) onChange([...arr, o.id]);
                  } else {
                    onChange(arr.filter((x) => x !== o.id));
                  }
                }}
              />
...
export function InputWizard({ projectId }: InputWizardProps) {
  ...
  const handleComplete = async (answers: Record<string, unknown>) => {
    ...
    for (const q of PHASE3_QUESTIONS) {
      const v = answers[q.id];
      if (v !== undefined && v !== "" && !(Array.isArray(v) && v.length === 0)) {
        phase3[q.id] = v;
      }
    }
    const required = PHASE3_QUESTIONS.map((q) => q.id);
    for (const key of required) {
      if (
        !(key in phase3) ||
        phase3[key] === "" ||
        (Array.isArray(phase3[key]) && (phase3[key] as unknown[]).length === 0)
      ) {
        phase3[key] = "unknown";
      }
    }
    ...
    try {
      await generatePhase3PositioningBrandTwoVariants({
        projectId,
        answersPhase3: { phase_3: phase3 },
        answersPhase1: undefined,
        answersPhase2: undefined,
      });
      router.refresh();
    } catch (e) {
  ...
  return (
    <ConfigDrivenWizard<Question>
      ...
      phaseId="phase_3"
      stepConfig={PHASE3_STEP_CONFIG}
      questions={PHASE3_QUESTIONS}
      loadAnswers={async () => ({})}
      saveAnswers={async () => {}}
      onComplete={handleComplete}
      lastStepButtonText="Varianten generieren"
