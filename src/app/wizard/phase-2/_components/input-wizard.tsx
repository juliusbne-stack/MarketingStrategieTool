...
import {
  ...
  PHASE2_STEP_CONFIG,
  type Question,
} from "./phase2-questions";
import { ConfigDrivenWizard } from "@/app/wizard/_shared/ConfigDrivenWizard";
import { generatePhase2GuidelinesThreeVariants } from "@/app/actions/phase2-actions";

interface InputWizardProps {
  projectId: number;
}

/** Validates a single answer for Phase 2 question types (single_choice, multi_choice, short_text). */
...
export function InputWizard({ projectId }: InputWizardProps) {
  ...
  const handleComplete = async (answers: Record<string, unknown>) => {
    ...
    const required = PHASE2_QUESTIONS.map((q) => q.id);
    for (const key of required) {
      if (
        !(key in phase2) ||
        phase2[key] === "" ||
        (Array.isArray(phase2[key]) && (phase2[key] as unknown[]).length === 0)
      ) {
        phase2[key] = "unknown";
      }
    }
    ...
    try {
      await generatePhase2GuidelinesThreeVariants({
        projectId,
        answersPhase2: { phase_2: phase2 },
