...
/**
 * Gets or creates a wizard session for the project and phase_4.
 */
export async function getOrCreatePhase4Session(projectId: number) {
  ...
  const existing = await db
    ...
    .where(
      and(
        ...
        eq(wizardSessionsTable.phaseId, PHASE_4)
      )
    )
...
/**
 * Lists the latest artifact per artifactKey for the given Phase 4 session.
 */
export async function listLatestPhase4Artifacts(
  projectId: number,
  sessionId: number
) {
...
/**
 * Gets the latest artifact version for a given Phase 4 session and artifactKey.
 */
export async function getLatestPhase4Artifact(
  projectId: number,
  sessionId: number,
  artifactKey: string
) {
