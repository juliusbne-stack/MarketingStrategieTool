...
/**
 * Gets or creates a wizard session for the project and phase_5.
 */
export async function getOrCreatePhase5Session(projectId: number) {
  ...
  const existing = await db
    ...
    .where(
      and(
        ...
        eq(wizardSessionsTable.phaseId, PHASE_5)
      )
    )
...
/**
 * Lists the latest artifact per artifactKey for the given Phase 5 session.
 */
export async function listLatestPhase5Artifacts(
  projectId: number,
  sessionId: number
) {
...
/**
 * Gets the latest artifact version for a given Phase 5 session and artifactKey.
 */
export async function getLatestPhase5Artifact(
  projectId: number,
  sessionId: number,
  artifactKey: string
) {
