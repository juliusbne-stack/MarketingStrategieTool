...
/**
 * Gets or creates a wizard session for the project and phase_2.
 */
export async function getOrCreatePhase2Session(projectId: number) {
  ...
  const existing = await db
    ...
    .where(
      and(
        ...
        eq(wizardSessionsTable.phaseId, PHASE_2)
      )
    )
...
/**
 * Lists the latest artifact per artifactKey for the given Phase 2 session.
 */
export async function listLatestPhase2Artifacts(
  projectId: number,
  sessionId: number
) {
...
/**
 * Gets the latest artifact version for a given Phase 2 session and artifactKey.
 */
export async function getLatestPhase2Artifact(
  projectId: number,
  sessionId: number,
  artifactKey: string
) {
